...
export default function Advisor() {
  ...
  return (
    <>
      <Head><title>AI Advisor — NEXUS-IT</title></Head>
      <AppLayout nickname={nickname}>
        <div className="advisor-chat">
          ...
          <div className="advisor-messages" ref={scrollRef}>
            {messages.length === 0 && <div className="advisor-empty">Ask for a project idea, feedback on your approach, or help getting unstuck.</div>}
            {messages.map((m, i) => (
              <div key={i} className={`advisor-msg ${m.role === 'user' ? 'user' : 'assistant'}`}>{m.content}</div>
            ))}
            {sending && <div className="advisor-msg assistant">Thinking…</div>}
          </div>
          {error && <div className="form-error" style={{ display: 'block', color: '#e35d5d', marginBottom: '10px' }}>{error}</div>}
          <div className="advisor-input-bar">
            <input type="text" placeholder="Ask something..." value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} disabled={sending} />
